...
from rwa_wdm.util import coerce_link_argument, parse_link_argument
...
DEFAULT_CONFIG = {
    ...
    "k": 3,
    "max_time_slots": 3,
    "model_path": "tmp/rwa_ppo/rwa_model_1200000_steps.zip",
    "log_dir": "tmp/rwa_ppo",
    "seed": None,
    "deterministic": True,
    "holding_time": 10,
    ...
    "inference_backend": "onnx",  # 'sb3' or 'onnx'
    "onnx_model_path": "tmp/rwa_ppo/rwa_policy_logits.onnx",
    "failure_lookup_path": "results_ppo/failure_lookup_COST239_1-3_k3.json",
    "failure_lookup_k": 3,
    "precompute_failure_lookup": False,
}
...
def parse_eval_args(argv: Sequence[str] | None = None) -> Namespace:
    ...
    parser.add_argument('--plot-topo', dest='plot_topo', action='store_true', help='Plot the topology before running simulations')
    parser.add_argument('--no-plot-topo', dest='plot_topo', action='store_false', help='Skip topology plotting')
    parser.add_argument('--failure-link', dest='failure_link', metavar='<src,dst>', type=parse_link_argument, help='Track blocking statistics for requests touching this link')
    ...
    parser.add_argument('--max-time-slots', type=int, default=DEFAULT_CONFIG['max_time_slots'], help='Max time-slot choices per request (t)')
    ...
    parser.add_argument('--inference-backend', choices=['sb3', 'onnx'], default=DEFAULT_CONFIG['inference_backend'], help='Policy inference backend')
    parser.add_argument('--onnx-model-path', default=DEFAULT_CONFIG['onnx_model_path'], help='ONNX model path used when inference-backend=onnx')
    parser.add_argument('--failure-lookup-path', default=DEFAULT_CONFIG['failure_lookup_path'], help='Optional JSON lookup table for failure-link impacted (src,dst) pairs')
    parser.add_argument('--failure-lookup-k', type=int, default=DEFAULT_CONFIG['failure_lookup_k'], help='K value used when precomputing failure-link lookup from KSP')
    parser.add_argument('--no-precompute-failure-lookup', dest='precompute_failure_lookup', action='store_false', help='Disable automatic in-memory precompute of failure lookup when no file is provided')
    ...
    parser.set_defaults(plot=DEFAULT_CONFIG['plot'], deterministic=DEFAULT_CONFIG['deterministic'], plot_topo=DEFAULT_CONFIG['plot_topo'], failure_link=DEFAULT_CONFIG['failure_link'], precompute_failure_lookup=DEFAULT_CONFIG['precompute_failure_lookup'])
...
def main(config: dict[str, Any] | None = None, argv: Iterable[str] | None = None) -> int:
    ...
    try:
        if config is not None:
            ...
            merged['failure_link'] = coerce_link_argument(merged.get('failure_link'))
