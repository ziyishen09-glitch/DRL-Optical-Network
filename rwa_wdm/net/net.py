...
class Lightpath(object):
    ...
    def reset_id_counter(cls) -> None:
        """Reset the global identifier counter (use when starting fresh episodes)."""
        cls._ids = count(0)
    ...
    def links(self) -> Iterable[Tuple[int, int]]:
        """Network links as a sequence of pairs of nodes (sliding window)"""
        for i in range(len(self._route) - 1):
            yield self._route[i], self._route[i + 1]
...
class Network(object):
    ...
    def plot_topology(self, bestroute: List[int] = None) -> None:
        ...
        for label, (i, j) in nodes.items():
            ...
            ax.annotate(label, xy=(i, j), ha='center', va='center', fontsize=16)
        ...
        # hide coordinate labels to remove numerical clutter
