...
def build_env_factory(args: Namespace) -> DummyVecEnv:
    ...
    _max_load = max(
        getattr(args, 'episode_load_range_max', 0.0) or 0.0,
        getattr(args, 'episode_load', 1.0),
        100.0,
    )

    # Build discrete load levels for one-hot encoding.
    # These must cover the FULL eval range (not just the training load),
    # so train and eval share the same observation-space dimension.
